...
enum keys {
    ...
    KEYCODE_NUMPAD_EQUAL = 161
};

static unsigned int map[49][2] = {{2306560, KEYMOD_SHIFT | KEYCODE_MOVE_HOME},//#2
...
char transform_for_modifier(char **write_onto, char *start, char start_len, unsigned int key_mod, char last_char) {
    char modifier;
    switch (key_mod) {
        default:
...
#define place_holder(a, b, c) if (0 == key_mod) { *write_onto = cursor_app ? (a) : (b);return 3;} else return transform_for_modifier(write_onto, "\033[1", 3, key_mod, (c));
#define place_holder_1(a, b) if (0 == key_mod) { *write_onto = (a);return 3;} else return transform_for_modifier(write_onto, "\033[1", 3, key_mod, (b));
...
char get_code(char **write_onto, unsigned int key_code, unsigned int key_mod, bool cursor_app, bool keypad_app) {
    ...
    switch (key_code) {
        ...
        case KEYCODE_DEL: {
            const char len = 1 + (0 != (key_mod & KEYMOD_ALT));
            char index = len;
...
char get_code_from_term_cap(char **write_onto, const char *string, char len, bool cursor_keys_application, bool keypad_application) {
    if (len > 3 || len < 2) return 0;
    int hash = string[0] << 16 | string[1] << 8 | (string[2] * (len != 2));
    char low = 0, high = 48, mid;
    ...
    return get_code(write_onto, key_code, key_mode, cursor_keys_application, keypad_application);
}
